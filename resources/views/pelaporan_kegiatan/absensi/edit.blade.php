        ...
        <div class="row">
          <div class="col-12">
            <div class="card card-primary"> 
                <form action="{{ route('absensi.update', ['absensi' => $absensi['id']]) }}" method="post" enctype="multipart/form-data">
                    <div class="card-body">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-sm-10">
                                <select name="id_sdm" class="form-control select2" required>
                                    ...
                                    @foreach($sdm as $key => $value)
                                        <option {{ $value->id == $absensi['id_sdm'] ? 'selected':'' }} value="{{ $value->id }}">{{ $value->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                               <select name="kehadiran" class="form-control" required>
                                <option {{ $absensi['kehadiran'] == 'Hadir' ? 'selected':'' }} value="Hadir">Hadir</option>
                                <option {{ $absensi['kehadiran'] == 'Izin' ? 'selected':'' }}  value="Izin">Izin</option>
                                <option {{ $absensi['kehadiran'] == 'Alpha' ? 'selected':'' }}  value="Alpha">Alpha</option>
                               </select>
                            </div>
                        </div>
