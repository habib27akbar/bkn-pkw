        ...
        <div class="row">
          <div class="col-12">
            <div class="card card-primary"> 
                <form action="{{ route('kegiatan.update', ['kegiatan' => $kegiatan['id']]) }}" method="post" enctype="multipart/form-data">
                    <div class="card-body">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-sm-10">
                                <select name="id_lokasi" id="id_lokasi" class="form-control">
                                    ...
                                    @foreach($lokasi as $key => $value)
                                        <option {{ $kegiatan['id_lokasi'] == $value->id ? 'selected':'' }} value="{{ $value->id }}">{{ $value->detail_lokasi.' Provinsi '.$value['nama_provinsi'].' Kabupaten Kota '.$value['nama_kabupaten_kota'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                           <label for="file" class="col-sm-2 col-form-label">Dokumen</label>
                            <div class="col-sm-10">
                                ...
                                <input type="hidden" name="dokumen_old" value="{{ $kegiatan['dokumen'] }}">
